...
//1106 #define get_hmv(mv) (mv)
//1106 #define set_hmv(mv) (mv)
// for convert hmvbest in hash
//1106 #define get_hmv(mv) ( (((int((mv>>7)/9)<<4)+((mv>>7)%9)) <<8) + (int((mv&127)/9)<<4)+((mv&127)%9) )
#define get_hmv(mv) ( ((((int(mv>>7)/9)<<4)+((mv>>7)%9)) <<8) + ((int(mv&127)/9)<<4)+((mv&127)%9) )
//#define set_hmv(mv) ( ((nRank(mv>>8)*9+nFile(mv>>8)) <<7) + (nRank(mv&255)*9+nFile(mv&255)) )
#define set_hmv(mv) ( (((mv>>12)*9+(mv>>8)&15)<<7) + ((mv&255)>>4)*9+  (mv&15) )   //1106  4bit from-rank, 4bit from-file, 4bit to-rank, 4bit to-file-->7bitfrom,7bitto
//
...
template<int nodetype>    
    int search(Board &board, int alpha, int beta, int depth, int null_allow); 
...
template<int nodetype>        
          int qsearch(Board &board, int alpha,int beta, int qdepth);
//1121    int quiesCheckPV(Board &board, int alpha,int beta, int qdepth); 
//1121    int quiesCheck(Board &board, int beta, int qdepth); 
...
*/    
    ...
    int ProbeHashQ(int beta, HashStruct* &hsho, Board &board, int &hashdepth);  //1007 return hashdepth  //1122 hsho  
    HashStruct* ProbeMove(Board &board);
//1122    int ProbeMoveQ(Board &board);
